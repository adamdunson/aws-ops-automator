...
INFO_ACCOUNT_SNAPSHOTS = "Processing set of {} snapshots for account {}"
...
DEBUG_SN_RETENTION_DAYS = "Deleting snapshot {} ({}) because it is older than retention period of {} days"
...
ERR_RETENTION_COUNT_FOR_COPIED = "Copied snapshots with volume-id {} can not be deleted by retention count in this version, " \
                                 "consider tagging copied snapshots for a deletion task that uses retention days instead"
...
class Ec2DeleteSnapshotAction:
    ...
    @staticmethod
    def can_execute(resources, parameters):
        volume_id = resources[0]["VolumeId"]
        if parameters .get(PARAM_RETENTION_COUNT, 0) > 0 and volume_id == "vol-ffffffff":
            raise ValueError(ERR_RETENTION_COUNT_FOR_COPIED.format(volume_id))
        if len(resources) > MAX_SNAPSHOTS:
            raise ValueError(ERR_MAX_RETENTION_COUNT_SNAPSHOTS.format(MAX_SNAPSHOTS, volume_id))

    @staticmethod
    def custom_aggregation(resources, parameters):
        if parameters.get(PARAM_RETENTION_COUNT, 0) > 0:
            # test if there is a marker tag with the original volume-id that is set by the copy action
            for res in resources:
                volume_id = res["VolumeId"]
                if volume_id == "vol-ffffffff":
                    volume_from_tag = res.get("Tags", {}).get(MARKER_TAG_SOURCE_VOLUME_ID, None)
                    if volume_from_tag is not None:
                        res["VolumeId"] = volume_from_tag
            ...
            def by_retention_days():
                ...
                for sn in sorted(self.snapshots, key=lambda s: s["Region"]):
                    ...
                    if snapshot_dt < delete_before_dt:
                        self.logger.debug(DEBUG_SN_RETENTION_DAYS, sn["SnapshotId"], sn["StartTime"], self.retention_days)
                        yield sn
            ...
            def by_retention_count():
                ...
                for sn in sorted_snapshots:
                    ...
                    if count_for_volume > self.retention_count:
                        self.logger.debug(DEBUG_SN_DELETE_RETENTION_COUNT, sn["SnapshotId"], count_for_volume)
                        yield sn
