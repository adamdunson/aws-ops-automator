...
class DynamodbSetCapacityAction(ActionBase):
    ...
    def _get_throughput_update(self, resp):
        ...
        if current_read_units != self._table_read_units_ or current_write_units != self._table_write_units_:
            update_args["ProvisionedThroughput"] = {
                "ReadCapacityUnits": self._table_read_units_,
                "WriteCapacityUnits": self._table_write_units_
            }

            self._logger_.info(INF_TABLE_UPDATE, self._table_name_, current_read_units, current_write_units,
                               self._table_read_units_,
                               self._table_write_units_)
        ...
        for index_name in gsi_updates:
            if index_name in global_secondary_indexes:
                ...
                current_gsi_read_units = index_throughput["ReadCapacityUnits"]
                current_gsi_write_units = index_throughput["WriteCapacityUnits"]
                ...
                if (update_read_units != current_gsi_read_units) or (update_write_units != current_gsi_write_units):
                    ...
                    self._logger_.info(INF_INDEX_UPDATE, index_name, current_read_units, current_write_units, update_read_units,
                                       update_write_units)
        ...
        self.result[actions.METRICS_DATA] = build_action_metrics(
            ...
            NewReadUnits=self._table_read_units_,
            NewWriteUnits=self._table_write_units_,
