...
MARKER_TAG_SOURCE_SNAPSHOT_ARN = "OpsAutomator:{}-RdsCopyClusterSnapshot-CopiedFromSnapshot"
MARKER_TAG_COPIED_TO = "OpsAutomator:{}-{}-RdsCopyClusterSnapshot-CopiedTo"
...
PARAM_DESC_SOURCE_ACCOUNT_TAG_ROLENAME = \
    "Name of the cross account role in the accounts that own a shared snapshot, that is used to read and write tags in these " \
...
PARAM_DESC_COPIED_SNAPSHOT_TAGS = "Copied tags from source snapshot. Use the TagOwnerAccountRoleName parameter to give access to " \
                                  "tags on source snapshot when copying shared snapshots."
...
PARAM_DESC_COPIED_SNAPSHOTS = \
    ...
    "shared snapshots the task needs access to the tags for the shared snapshot in the owner account, and a role " \
    "must be provided in parameter TagOwnerAccountRoleName to give the task access to these tags in the source account."
...
PARAM_DESC_ACCOUNTS_RESTORE_PERMISSIONS = "List of account that will be granted access to restore RDS clusters from the copied " \
                                          "snapshot."
PARAM_DESC_KMS_KEY = \
    "The AWS KMS key for an encrypted cluster snapshot. The KMS key ID is the Amazon Resource Name (ARN), KMS key identifier, " \
...
INFO_ACCOUNT_SNAPSHOT = "Copying RDS cluster snapshot {} for account {} from region {} to region {}"
...
class RdsCopyClusterSnapshotAction(ActionBase):
    """
    Class implements action for copying EC2 Snapshots
    """

    properties = {
        ACTION_TITLE: "RDS Copy Cluster Snapshot",
        ACTION_VERSION: "1.0",
        ACTION_DESCRIPTION: "Copies RDS cluster snapshot",
        ACTION_AUTHOR: "AWS",
        ACTION_ID: "de3c487a-d22f-4a77-a24e-21f8469a74f8",
        ...
        # RDS CopySnapshot only allows 5 concurrent copies per account to a destination region
    ...
    def __init__(self, action_arguments, action_parameters):
        ...
        self.source_snapshot_id = self.db_snapshot["DBClusterSnapshotIdentifier"]
        self.source_snapshot_arn = self.db_snapshot["DBClusterSnapshotArn"]
        ...
        source_db_cluster = self.db_snapshot.get("DBClusterIdentifier", None)
        if source_db_cluster is None:
            source_db_cluster_from_tag = self.db_snapshot.get("Tags", {}).get(
                MARKER_RDS_TAG_SOURCE_DB_CLUSTER_ID.format(os.getenv(handlers.ENV_STACK_NAME)), None)
            if source_db_cluster_from_tag is not None:
                source_db_cluster = source_db_cluster_from_tag
        self.source_volume_id = source_db_cluster
    ...
    def process_and_select_resource(service, logger, resource_name, resource, context, task, task_assumed_role):
        ...
        # source snapshot
        db_snapshot_arn = resource["DBClusterSnapshotArn"]
    ...
    def is_completed(self, snapshot_create_data):
        ...
        def grant_restore_permissions(snapshot):
            ...
            if self.accounts_with_restore_permissions is not None and len(self.accounts_with_restore_permissions) > 0:
                ...
                if snapshot.get("StorageEncrypted", False):
                    default_rds_key_arn = self.get_kms_key("alias/aws/rds").get("Arn")
                    if snapshot.get("KmsKeyId") == default_rds_key_arn:
                        self._logger_.error(ERR_CAN_NOT_SHARE_ENCRYPTED_DEFAULT_KEY, snapshot["DBClusterSnapshotIdentifier"])
                        return
                ...
                try:
                    self.rds_destination_client.modify_db_cluster_snapshot_attribute_with_retries(**args)
        ...
        def set_source_snapshot_tags(copy_id):
            ...
            if len(snapshot_tags) > 0:
                ...
                if source_client is None:
                    ...
                    return
                tagging.set_rds_tags(source_client,
                                     tags=snapshot_tags,
        ...
        try:
            copied_snapshot = rds.get(services.rds_service.DB_CLUSTER_SNAPSHOTS,
                                      region=self.destination_region,
                                      DBClusterSnapshotIdentifier=copied_snapshot_id,
                                      _expected_boto3_exceptions_=["DBClusterSnapshotNotFoundFault"])
        except Exception as ex:
            if "DBClusterSnapshotNotFoundFault" in ex.message:
    ...
    def action_logging_subject(arguments, _):
        ...
        snapshot_id = db_snapshot["DBClusterSnapshotIdentifier"]
    ...
    def execute(self):
        ...
        def get_tags_for_copied_rds_snapshot():
            ...
            snapshot_tags[MARKER_RDS_TAG_SOURCE_DB_CLUSTER_ID.format(os.getenv(handlers.ENV_STACK_NAME))] = self.source_volume_id
        ...
        args = {
            ...
            "SourceDBClusterSnapshotIdentifier": self.source_snapshot_arn,
        ...
        # start the copy
        resp = self.rds_destination_client.copy_db_cluster_snapshot_with_retries(**args)

        # id of the copy
        copied_snapshot_id = resp.get("DBClusterSnapshot", {}).get("DBClusterSnapshotIdentifier")
        copied_snapshot_arn = resp.get("DBClusterSnapshot", {}).get("DBClusterSnapshotArn")
        ...
        copied_tag_name = MARKER_TAG_COPIED_TO.format(os.getenv(handlers.ENV_STACK_NAME)[0:128], self._task_)
