...
def set_redshift_tags(redshift_client, resource_arns, tags, can_delete=True, logger=None):
    def create_tags(client, resources, created_tags):
        for arn in resources:
            client.create_tags_with_retries(ResourceName=arn, Tags=created_tags)

    def delete_tags(client, resources, deleted_tags):
        for arn in resources:
            client.delete_tags_with_retries(ResourceName=arn, TagKeys=deleted_tags)

    _set_resource_tags(client=redshift_client,
                       resources=resource_arns,
                       tags=tags,
                       create_func=create_tags,
                       delete_func=delete_tags,
